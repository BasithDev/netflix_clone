...
const TvShows = () => {
  ...
  const [tvShows, setTvShows] = useState([]); // Stores the fetched TV shows
  ...
  const fetchTvShows =useCallback( async () => {
    try {
      ...
      setTvShows(response.data.results); // Set the TV shows for the current page
  ...
  return (
    <div className="px-4 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8">
        {tvShows.map((show) => (
          <div
            onClick={() => navigate(`/${'tv'}/${show.id}`)} // Wrap in arrow function
            key={show.id}
